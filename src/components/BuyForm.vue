<template>
  <div class="buy-form">
    ...
    <form event="none" class="buy-form__form form" @submit="buy"
          :style="{backgroundImage: `url('${require(`st/images/buy_form/${type}.jpg`)}')`}">
      <p class="form__title">
        Цена: <span class="form__price">{{ Math.round(info.price * 1.4) }} ₽</span><span
          class="form__real-price">{{ info.price }} ₽</span>
      </p>
      <AppInput class="form__input" :type="'email'" :required="true" :placeholder="'Введите Email'" :value.sync="email"/>
      <p class="buy-form__text form__after-buy-text">
        После оплаты, данные аккаунта придут вам на почту
      </p>
      <PackButton class="form__pack-button" :price="info.price" :type="type"/>
    </form>
    <button type="button" class="buy-form__close" @click="$emit('close')" />
  </div>
</template>
